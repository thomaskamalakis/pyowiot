...
from libow3 import plot_on_grid, spectra, constants, sensor_net, grid_of_points, \
                   aligned_to, sensor_consumption, interp_on_grid, half_angle_to_order
from scipy.special import erfc, erfcinv

HOME_DIR = '/home/thomas/Documents/jocn_paper/figures/'

Phi_sensor = np.pi / 3.0
m_sensor = half_angle_to_order(Phi_sensor)
...
master = {
        'rS' : np.array([L/2, W/2, H]),
        'rR' : np.array([L/2, W/2, H]),
        'FOV' : np.pi/2.0,
        'A' : 1e-4,
        'm' : 1,
        'nS' : -constants.ez,
...
plt.colorbar()
plt.axis('equal')
plt.savefig(HOME_DIR + 'PT0_2.png')
plt.show()
...
plt.colorbar()
plt.axis('equal')
plt.savefig(HOME_DIR + 'ID0_2.png')
plt.show()

# Downlink SNR
snd_dB = 10 * np.log10( sn.snr_d )
plt.figure()
plot_on_grid(sn.sensors['rR'], snd_dB.reshape(-1), x, y, 
             p1 = constants.ex,
             p2 = constants.ey)

plt.title('$\mathrm{SNR}$ [dB]')
plt.xlabel('$x$ [m]')
plt.ylabel('$y$ [m]')
plt.colorbar()
plt.axis('equal')
plt.savefig(HOME_DIR + 'SNRd_2.png')
plt.show()
...
plot_arrow(ax2, x, tBL10, 30, 0.5, 50, 'r')
plot_arrow(ax2, x, tBL60, 30, 0.5, 25, 'g')

ax2.set_ylabel('$t_\mathrm{BL}$ [days]')
lgd = plt.legend(handles = [p1, p2, p3], bbox_to_anchor=(1.1, 1), loc='upper left')
plt.savefig(HOME_DIR + 'tBL2.png',
            bbox_extra_artists=[lgd], bbox_inches='tight')
plt.show()

plt.figure()
plot_on_grid(sn.sensors['rR'], sn.Iamb_s.reshape(-1), x, y, 
             p1 = constants.ex,
             p2 = constants.ey)
